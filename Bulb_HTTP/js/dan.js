var dan = (function () {
    ...
    var POLLING_INTERVAL = 250;   // was 500
    var _pull;
    var _push;
    var _mac_addr = '';
    var _profile = {};
    ...
    function register (endpoint, profile, callback) {
		// change name to Display -- modify by tsaiwn@cs.nctu.edu.tw 
		// profile['d_name'] = (Math.floor(Math.random() * 99)).toString() + '.' + profile['dm_name'];
		heading = csmapi.get_endpoint().charAt(8).toUpperCase();
		h234 = csmapi.get_endpoint().substring(9, 12);
        profile['d_name'] = heading+h234 +"_"+(Math.floor(Math.random() * 999)).toString() + '.' + profile['dm_name'];
        _profile = profile;
